...
main = hspec $ do
    ...
    describe "executeInstruction" $ do
        it "executes set" $ do
            let (State _ regs _) = executeInstruction emptyState (Set 'a' "1")
            (Map.!) regs 'a' `shouldBe` 1
        it "executes sub" $ do
            let s1 = executeInstruction emptyState (Set 'a' "1")
            let (State _ regs _) = executeInstruction s1 (Sub 'a' "2")
            (Map.!) regs 'a' `shouldBe` (-1)
        it "executes mul" $ do
            let s1 = executeInstruction emptyState (Set 'a' "2")
            let (State _ regs _) = executeInstruction s1 (Mul 'a' "2")
            (Map.!) regs 'a' `shouldBe` 4
        it "executes sub on empty" $ do
            let (State _ regs _) = executeInstruction emptyState (Sub 'a' "2")
            (Map.!) regs 'a' `shouldBe` (-2)
        it "executes jnz (reg)" $ do
            let s1 = executeInstruction emptyState (Set 'a' "1")
            let (State p _ _) = executeInstruction s1 (Jnz "a" "3")
            p `shouldBe` 4
        it "ignores jnz (reg is zero)" $ do
            let (State p _ _) = executeInstruction emptyState (Jnz "a" "3")
            p `shouldBe` 1 -- next
        it "executes jnz (value)" $ do
            let (State p _ _) = executeInstruction emptyState (Jnz "1" "3")
            p `shouldBe` 3
        it "ignores jnz (value is zero)" $ do
            let (State p _ _) = executeInstruction emptyState (Jnz "0" "3")
            p `shouldBe` 1 -- next
