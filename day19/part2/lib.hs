...
walk :: Rows -> Position -> Int -> (Position, Int)
walk rows position@(Position r c dir) count = do
    if isFinished rows position then
        ...
        where
            ...
            continue row ch
                | ch == '+'       = walk rows (turn rows position) (count + 1)
                | isAsciiUpper ch = walk rows newPos (count + 1)
                | otherwise       = walk rows newPos (count + 1)

countSteps :: [String] -> Int
countSteps lines = do
    let rows = Seq.fromList (map toRow lines)
    let pos = startingPosition rows
    let (p, cnt) = walk rows pos 0
    cnt
